...
db_name = os.path.splitext(initFile)[0]+".pdl"

import PyDbLite

folder_db = PyDbLite.Base(os.path.join(rootDir,"admin",db_name))
folder_db.create("folder","level","alias",mode="open")
if not folder_db():
    # initiate folder db with document root
    folder_db.insert(folder=rootDir,alias="")
    # initiate aliases
    folder_db.insert(folder=os.path.join(rootDir,"admin"),alias="admin",level=2)
    folder_db.insert(folder=os.path.join(rootDir,"doc"),alias="doc")
    folder_db.commit()

# protected zones : for all scripts, execute Login(role=level)
protectedDirs = [ r["folder"] for r in folder_db if r["level"] ]

# aliases
alias=dict([(r["alias"],r["folder"]) for r in folder_db if r["alias"]])

...
class BaseScript:
    ...
    def render(self,nameSpace={},**args):
        ...
        # if script in protected zone, prepend function Login() to test
        # if user is the site administrator
        pythonCode=self.pythonCode()
        for path,level in k_config.protectedDirs.iteritems():
            if level=="all":
                continue
            (protectedDir,depth) = k_utils.pathInDirs(self.name,[path])
            if protectedDir is not None:
                self.code='Login(role=["%s"]);' %level + self.pythonCode()
                break
        ...
        try:
            ...
            # if show_trace is False, just say there was a problem
            status=0
            if not k_config.show_trace:
                sys.stdout=StringIO()
                sys.stdout.write("The server was unable to process the request")
            else:
                # create an instance of the Error class in module k_debugger
                # for traceback printing and debugging
                key=k_utils.generateRandom(8)
                error=k_debugger.Error(tree,
    ...
    def rel(self,path):
        """Function to build the correct URL inside functions where arguments 
        are passed under the "smart url" form"""
        return self.up + path

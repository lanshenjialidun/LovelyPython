...
def protection(path,protected):
    """Return protection level for path
    protected is a dictionary folder:protection level
    if path is in folder or a subfolder, return protection level
    else return "all"
    """
...
# ============================================================================
# function used to check if a user is authenticated
# uses the default authent scheme : if the cookie has keys "login" and "skey",
# check if the PyDbLite base "users.pdl" in the data folder has a record with
# login = login and session_key == skey
# ============================================================================

def is_logged(cookie,host):
    """check if user is authenticated"""
    import k_config
    for k in "skey","login":
        if not k in cookie.keys():
            return False
    # search in users database to see if session key is correct
    import PyDbLite
    db = PyDbLite.Base(os.path.join(k_config.serverDir,"data","users.pdl"))
    db.open()
    ...
    user = user[0]
    import datetime
    db.update(user,nb_visits = user["nb_visits"]+1,
        last_visit = datetime.datetime.now())
    db.commit()
    return True,user["role"]

def has_admin(host):
    """Check if an administrator is defined for the host"""
    import os
    import k_config
    import PyDbLite
    users = os.path.join(k_config.serverDir,"data","users.pdl")
    if os.path.exists(users):
        db = PyDbLite.Base(users).open()
        if db(host=host,role="admin"):
            return True
    return False

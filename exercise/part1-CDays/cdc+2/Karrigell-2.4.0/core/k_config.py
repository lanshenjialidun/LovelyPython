...
def usage():
    print "Usage : python Karrigell.py [-P port] [-S] [-H] [initFile]"
    print "\n\tport = HTTP port"
...
if _args:
    if len(_args) == 1:
        ...
        raise ValueError,"No argument for init file, args = %s" %str(_args)
        #usage()
        #sys.exit(2)
...
try:
    reload_modules = conf.getboolean("Server","reload_modules")
except:
    pass
...
try:
    ...
    print "Unknown output_encoding in configuration file : %s" %output_encoding
    sys.exit()

# form data encoding
try:
    encode_form_data = conf.getboolean("Server","encode_form_data")
except (ConfigParser.NoSectionError,ConfigParser.NoOptionError):
    pass
...
try:
    ...
    protectedDirs = dict([ (os.path.normpath(_p),"admin") for _p in protectedDirs ])
except (ConfigParser.NoOptionError,ConfigParser.NoSectionError):
    pass
except:
    traceback.print_exc(sys.stderr)
...
for path in ["core","modules","debugger"]:
    ...
    if not p in sys.path:
        sys.path.append(p)
for f in os.listdir(os.path.join(serverDir,"core","modules")):

...
"""This cgi script is run by Apache for all requests for documents
in the Karrigell folder"""
...
# set current directory to webapps
os.chdir(os.path.dirname(os.path.dirname(os.getcwd())))
sys.path.append(os.getcwd())
# set argv for k_config
...
class RequestHandler(KarrigellRequestHandler.KarrigellRequestHandler,
    ...
    def __init__(self, server, request, client_address):
        ...
        self.requestline = "%s %s %s" %(env["REQUEST_METHOD"],
            env["SCRIPT_URL"],env["SERVER_PROTOCOL"])
    ...
    def redirect(self,url):
        ...
        Override KarrigellRequestHandler to send Status header"""

...
base_name = "test"
name = "cities"
fields = (("rec_id","int(11)","input"),("name","text","input"),("country","text","input"))
__id__ = "rec_id"
import os
...
user_test = hasattr(Session(),'cities_user')
...
def editRecord(record_id):
    ...
    print TR(TD("country")+TD(INPUT(name="country",size="40",value=record["country"])))
...
def insertRecord(record_id,**untyped_fields):
    ...
    sql = unicode(sql,k_config.output_encoding)
    cursor.execute(sql)
    connection.commit()
    connection.close()
    raise HTTP_REDIRECTION,"index"
...
def check_login(**user_data):
    digest=open("cities.ini","rb").read()

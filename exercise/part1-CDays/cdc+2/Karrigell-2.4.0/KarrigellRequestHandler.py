...
# change sys.path for imports
dirs = ["core","debugger","databases"]
for subdir in ["modules","localize","parsers"]:
    dirs.append(os.path.join("core",subdir))
for path in dirs:
...
import k_config
import Template
import URLResolution, k_utils, k_script, modify_request
import k_debugger
import k_session
...
# lock to make Karrigell thread-safe
import threading
lock = threading.Lock()
...
class KarrigellRequestHandler(LocalizedRequestHandler):
    ...
    def prepare_env(self):
        ...
        if k_config.encode_form_data:
            ...
            qs_encoding = k_encoding.try_encoding(urllib.unquote(self.qs), 
               ['ascii', k_config.output_encoding, 'utf-8', 'iso8859_1_ncc', 
               'cp1252', 'macroman'])
            ...
            for key in self.body.keys():
                ...
                for i,elt in enumerate(self.QUERY[key]):
                    if not elt.filename:
                        ...
                        # output_encoding, but unfortunately there are some 
                        # broken browsers so we have to do the test...
                        # for backward compatibility, if output_encoding=='',
                        # don't use unicode at all
                        if k_config.encode_form_data:
                            s_encoding = k_encoding.try_encoding(s, 
                                ['ascii', k_config.output_encoding, 'utf-8', 
    ...
    def try_handle_data(self):
        ...
        # if fileName is a directory, search an index file and redirect
            ...
            try:
                ...
                if not k_config.allow_directory_listing:
        ...
        if script:
            ...
            # execution
            self.execute(script)
        elif k_utils.protection(fileName,k_config.protectedDirs) != "all":
            # static file in a protected directory
            level = k_utils.protection(fileName,k_config.protectedDirs)
            self.prepare_env()
            self.nameSpace['fileName'] = fileName
            script = Template.getScript(
                os.path.join(k_config.serverDir,'core','dump.py'))
            script.code = 'Login(role=["%s"]);' %level+ script.code
            self.execute(script)
        else:
    ...
    def authenticate(self,realm,errorMessage):
        ...
        self.end_headers()
        self.wfile.write(k_encoding.encode(errorMessage, k_config.output_encoding))
    ...
    def Login(self,script="/login.ks/login",role=[],baseurl=None,path=None):
        """If user is not logged, redirect to the login script"""
        if k_utils.is_logged(self.COOKIE,self.host):
            if not role:
                return # user logged, all roles accepted
            if self.COOKIE["role"].value in role:
        ...
        path = path or self.path_without_qs
        self.redirect("%s?baseurl=%s&path=%s&role=%s" 
                %(script,baseurl,path,",".join(role)))

    def Logout(self,text="Logout"):
        """Return a string with a link to the script logout.py to erase the
        login cookie, then go back to the page"""
        if k_utils.is_logged(self.COOKIE,self.host):
            return '<a href="/login.ks/logout?baseurl=%s&path=%s">%s</a>' \
                %(self.baseurl,self.path_without_qs,text)
        else:
            return ''
    ...
    def send_error(self, code, message=None):
        ...
        self.wfile.write(k_encoding.encode(err_msg, k_config.output_encoding))
